...
export class SerialportronComponent implements OnInit {
  ...
  scan() {
    ...
    const me = this;
    this.electronService.ipcRenderer.send("mainWindowLoaded");
    this.electronService.ipcRenderer.on("resultSent", function (evt, result) {
      me.list = [];
      console.log('kalsi' + result)
      for (var i = 0; i < result.length; i++) {
        me.list.push(result[i].name);
      }
